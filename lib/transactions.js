 ...
 */
    // some coin need 2txs for pay to the founder  modify by mrzeta
    // foundersReward, DevRewardStartBlock, DevRewardStopBlock, foundersRewardAddress,
var generateOutputTransactions = function(poolRecipient, foundersReward, DevRewardStartBlock, DevRewardStopBlock, foundersRewardAddress, recipients, rpcData){

    var reward = rpcData.coinbasevalue;
    var rewardToPool = reward;
    // some coin need 2txs for pay to the founder  modify by mrzeta
    var txOutputBuffers = [];
    if(foundersReward===true){
        if (rpcData.height >= DevRewardStartBlock && rpcData.height <= DevRewardStopBlock) {
            foundersRewardAddress = foundersRewardAddress || {};
            for (var xr in foundersRewardAddress){
                var payeeScriptpfa = util.addressToScript(xr);
                txOutputBuffers.push(Buffer.concat([
                    util.packInt64LE(foundersRewardAddress[xr]),
                    util.varIntBuffer(payeeScriptpfa.length),
                    payeeScriptpfa
                ]));
            };
            // // payeeScriptpfa = null;
    ...
    if (rpcData.lmnode) {
        if (rpcData.lmnode.payee) {
            var payeeReward = 0;
            payeeReward = rpcData.lmnode.amount;
            var payeeScript = util.addressToScript(rpcData.lmnode.payee);
            txOutputBuffers.push(Buffer.concat([
                util.packInt64LE(payeeReward),
                util.varIntBuffer(payeeScript.length),
                payeeScript
    ...
    /* nor masternode */
    if(rpcData.noirnode_payments_started===true && rpcData.height >= 262100){
        if (rpcData.noirnode.payee && rpcData.noirnode.amount) {
            var norPayee = rpcData.noirnode.payee;
            var norAmount = rpcData.noirnode.amount;
            // var norPayee = 'ZJEPPqytETrAdqHtHkADH9Rs3ukuBSGNpX';
            // var norAmount = '76a91443ebb1a8b54f8a4c7b8ac8b93e4f0a56cc9fd31288ac';
            //console.log('going to send '+norAmount+' to '+norPayee);
            var payeeScript = util.addressToScript(norPayee);
            txOutputBuffers.push(Buffer.concat([
                util.packInt64LE(norAmount),
                util.varIntBuffer(payeeScript.length),
            payeeScript
            ]));
        }
    ...
    // some coin need 2txs for pay to the founder  modify by mrzeta
    // foundersReward, DevRewardStartBlock, DevRewardStopBlock, foundersRewardAddress,
exports.CreateGeneration = function(rpcData, publicKey, extraNoncePlaceholder, reward, txMessages, foundersReward, DevRewardStartBlock, DevRewardStopBlock, foundersRewardAddress, recipients){
    ...
    var txComment = txMessages === true ?
        util.serializeString('https://github.com/mrzeta/node-stratum-pool') :
        new Buffer([]);
    ...
    var scriptSigPart2 = util.serializeString('/meetrue.com/');
    ...
    // some coin need 2txs for pay to the founder  modify by mrzeta
    // foundersReward, DevRewardStartBlock, DevRewardStopBlock, foundersRewardAddress,
    var outputTransactions = generateOutputTransactions(publicKey, foundersReward, DevRewardStartBlock, DevRewardStopBlock, foundersRewardAddress, recipients, rpcData);
